...
export default async function AdminAnalyticsPage() {
  ...
  return <div><h1 className="text-3xl font-extrabold text-white">Product analytics</h1><p className="mt-2 text-sm text-slate-400">First-party events from the last 30 days. No IP addresses or personal profiles are stored.</p><div className="mt-8 grid gap-5 @min-[640px]:grid-cols-2 @min-[1024px]:grid-cols-4">{summary.map((item) => <Card key={item.event_name} className="p-6"><p className="text-3xl font-extrabold text-white">{item.count}</p><p className="mt-2 text-sm text-slate-400">{labels[item.event_name] || item.event_name}</p></Card>)}{summary.length === 0 && <Card className="p-6 text-sm text-slate-400">No analytics events recorded yet.</Card>}</div></div>;
}
